 ...
 */
export function extractChunksGoogle(source: Document | Element | string, url, options: any = {}) {
  const {
    minTextLength = 100,
    maxChunks = 50,
    includeAds = false
  ...
  const filteredChunks = includeAds 
    ? chunks 
    : chunks.filter((chunk) => !inferAdvert(chunk, url));
 ...
 */
function extractGoogleChunk(element: Element, linkRegex, pageUrl, options: any = {}) {
  ...
  const imgAlt = imgEl ? (imgEl as HTMLImageElement).alt.trim() : '';
