...
export function assMatch(value: any, type: any, message?: string): void {
	if (type === String) {
		...
		// Handle string type names like "String[]", "?String"
		if (type === 'String[]') {
			if (!Array.isArray(value)) {
				throw new Error(message || `Expected string array, got ${typeof value}`);
			}
			for (const item of value) {
				if (typeof item !== 'string') {
					throw new Error(message || `Expected string array, got array with non-string items`);
				}
			}
		} else if (type === '?String') {
